""" The model of Image Enhancer implemented with WGAN """
...
class Enhancer(object):
    ...
    def load_data(self, process=False):
        ...
        if process:
            ...
            self.source['process'] = (self.source['process'].astype('float32') - 127.5) / 127.5
            return
        self.source['train'], self.source['valid'], self.source['test'] = load_img(self.img_dir, self.img_shape)
        print('Preprocessing data...')
        self.source['train'] = (self.source['train'].astype('float32') - 127.5) / 127.5
        self.source['valid'] = (self.source['valid'].astype('float32') - 127.5) / 127.5
        self.source['test'] = (self.source['test'].astype('float32') - 127.5) / 127.5
    ...
    def train_model(self, critic_updates=5):
        ...
        self.d_model.compile(RMSprop(lr=self.learning_rate), loss=wasserstein_loss)
        self.d_model.trainable = False
        self.gan.compile(RMSprop(lr=self.learning_rate), loss=[binary_crossentropy, wasserstein_loss], loss_weights=[10, 1])
        self.d_model.trainable = True
        ...
        for itr in range(self.epoch):
            ...
            for idx in range(int(train_num / self.batch_size)):
                ...
                for _ in range(critic_updates):
                    for l in self.d_model.layers:
                        weights = l.get_weights()
                        weights = [np.clip(w, -0.01, 0.01) for w in weights]
                        l.set_weights(weights)
                    d_loss_real = self.d_model.train_on_batch(raw_batch, -np.ones((self.batch_size, 1)))
                    d_loss_fake = self.d_model.train_on_batch(generated, np.ones((self.batch_size, 1)))
                    d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
                    d_losses.append(d_loss)
                    print('D real loss: %s, fake loss: %s' % (d_loss_real, d_loss_fake))
                print('D loss: %s' % np.mean(d_losses))
                self.d_model.trainable = False
    ...
    def save_image(self, name, num=10):
        ...
        """
        raw = 0.5 * self.source['test'] + 0.5
        crp = 0.5 * self.corrupted['test'] + 0.5
        ...
        for i in range(num):
            ...
            plt.imshow(raw[i].reshape(self.img_shape))
            plt.axis('off')
        for i in range(num):
            plt.subplot(3, num, i + num + 1)
            plt.imshow(crp[i].reshape(self.shape['in']))
            plt.axis('off')
        for i in range(num):
...
class AbsModel(object):
    ...
    def conv_pool(self, layer, filters):
        ...
        """
        layer = Conv2D(filters, (3, 3), padding='same', strides=(2, 2), use_bias=False)(layer)
    ...
    def conv(self, layer, filters, shrink=False):
        ...
        layer = BatchNormalization()(layer)
        layer = self.activate(layer)
        layer = Conv2D(filters, (3, 3), padding='same', strides=((2, 2) if shrink else (1, 1)), use_bias=False)(layer)
        return layer
